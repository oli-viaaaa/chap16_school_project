...
public class SchoolManagementMain {
	 ...
	 * @return
	 */
	private static List<Student> getStudentByDept(List<Student> studentList, List<Department> departmentList,
			String deptName) {
	...
	/**
	 * 서울에 사는 남학생을 찾아서 ArrayList로 반환해주는 메소드 2000이전 주민번호
	 * 
	 * @param studentList
	 * @return
	 */
	...
	private static void printAllStudent(List<Student> studentList) {
		for (Student student : studentList) {
			System.out.println(student.getId() + " " + student.getName() + " " + student.getJumin() + " "
					+ student.getYear() + " " + student.getAddress() + " " + student.getDepartment());
		}
	}
